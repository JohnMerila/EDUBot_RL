  ...
  if (GT_pose_initialized)
  {
    ...
    if (rf2o_ref.first_laser_scan == false)
    {
      if (new_scan->ranges.size() != rf2o_ref.width)
      {
        RCLCPP_WARN_THROTTLE(
          ...
          "Ignoring scan with %zu ranges; RF2O initialized with %u",
    ...
    else
    {
      ...
      if (!setLaserPoseFromTf())
      {
        RCLCPP_WARN_THROTTLE(
          get_logger(), *get_clock(), 5000,
          "Waiting for transform from '%s' to '%s' before initializing RF2O",
...
void CLaserOdometry2DNode::process()
{
  if (!rf2o_ref.is_initialized())
  {
    RCLCPP_WARN_THROTTLE(
      get_logger(), *get_clock(), 5000,
      "Waiting for the first LaserScan and base-to-laser transform...");
    return;
  }

  // The processing loop can run faster than the LiDAR. Waiting between two
  // valid scans is normal and must not be reported as a sensor failure.
  if (!scan_available())
    return;
